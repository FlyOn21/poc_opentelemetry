...
VICTORIATRACES_DS_UID = os.getenv("VICTORIATRACES_DS_UID", "victoriatraces")
...
def _build_tempo_url(trace_id_hex: str) -> str:
    """
    Constructs Grafana Explore URL for viewing trace via Jaeger datasource proxy.

    :param trace_id_hex: A string representing the hexadecimal trace ID to be queried.
    :return: A string containing the URL for accessing the trace in Grafana.
    """
    return f"{GRAFANA_BASE_URL}/api/datasources/proxy/uid/{VICTORIATRACES_DS_UID}/api/traces/{trace_id_hex}"
